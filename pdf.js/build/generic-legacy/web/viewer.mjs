...
class EditorUndoBar {
  ...
  static enableNotifications(enabled = true) {
    EditorUndoBar.#notificationsEnabled = enabled;
  }
  destroy() {
...
;// ./web/pdf_recent_files_viewer.js
...
class PDFRecentFilesViewer {
  ...
  _renderFilesList() {
    ...
    if (this._currentFilter) {
      files = files.filter(file => file.filename.toLowerCase().includes(this._currentFilter) || file.url.toLowerCase().includes(this._currentFilter));
    }
  ...
  _showFileInfo(file) {
    const info = [`Filename: ${file.filename}`, `Last Opened: ${this._formatDate(file.lastOpened)}`, `Current Page: ${file.viewState.page}`, `Zoom Level: ${Math.round(file.viewState.zoom * 100)}%`, `Rotation: ${file.viewState.rotation}°`, `Has Edits: ${file.hasEdits ? 'Yes' : 'No'}`, `File Path: ${file.url}`].join('\n');
    alert(info);
  }
}
...
class PDFSidebar {
  ...
  constructor({
    ...
    this.recentFilesButton = elements.recentFilesButton;
    this.thumbnailView = elements.thumbnailView;
    this.outlineView = elements.outlineView;
    this.attachmentsView = elements.attachmentsView;
    this.layersView = elements.layersView;
    this.recentFilesView = elements.recentFilesView;
  ...
  reset() {
    ...
    this.outlineButton.style.setProperty("display", "none", "important");
    this.outlineButton.disabled = true;
    this.attachmentsButton.style.setProperty("display", "none", "important");
    this.attachmentsButton.disabled = true;
    this.layersButton.style.setProperty("display", "none", "important");
    this.layersButton.disabled = true;
  ...
  switchView(view, forceOpen = false) {
    ...
    switch (view) {
      ...
      case SidebarView.OUTLINE:
        if (this.outlineButton.style.getPropertyValue("display") === "none") {
          return;
        }
        break;
      case SidebarView.ATTACHMENTS:
        if (this.attachmentsButton.style.getPropertyValue("display") === "none") {
          return;
        }
        break;
      case SidebarView.LAYERS:
        if (this.layersButton.style.getPropertyValue("display") === "none") {
          return;
        }
        break;
    ...
    toggleCheckedBtn(this.recentFilesButton, view === SidebarView.FILES, this.recentFilesView);
  ...
  open() {
    ...
    this.isOpen = true;
    toggleExpandedBtn(this.toggleButton, !this.#isTemporarilyOpen);
  ...
  #addEventListeners() {
    ...
    this.sidebarContainer.addEventListener("mouseenter", () => {
    ...
    this.sidebarContainer.addEventListener("mouseleave", evt => {
      const toElement = evt.relatedTarget;
      if (toElement === this.resizer) {
        return;
      }
      this.#isMouseInSidebar = false;
      if (this.#isTemporarilyOpen) {
        this.#hoverTimeoutId = setTimeout(() => {
          if (!this.#isMouseInSidebar && !this.#isMouseInResizer) {
            this.#isTemporarilyOpen = false;
            this.close();
          }
        }, 200);
    ...
    this.resizer.addEventListener("mouseenter", () => {
      this.#isMouseInResizer = true;
      this.#hoverStartTime = Date.now();
      if (this.#hoverTimeoutId) {
        clearTimeout(this.#hoverTimeoutId);
        this.#hoverTimeoutId = null;
      }
      if (!this.isOpen && !this.#isTemporarilyOpen) {
        this.#hoverTimeoutId = setTimeout(() => {
          const hoverDuration = Date.now() - this.#hoverStartTime;
          if (!this.isOpen && this.#isMouseInResizer && hoverDuration >= this.#HOVER_DELAY) {
    ...
    this.resizer.addEventListener("mouseleave", evt => {
      this.#isMouseInResizer = false;
      ...
      const toElement = evt.relatedTarget;
      if (toElement && this.sidebarContainer.contains(toElement)) {
        return;
      }
      if (this.#isTemporarilyOpen) {
        this.#hoverTimeoutId = setTimeout(() => {
    ...
    this.sidebarContainer.addEventListener("click", () => {
      if (this.#isTemporarilyOpen && this.#hoverTimeoutId) {
        clearTimeout(this.#hoverTimeoutId);
        this.#hoverTimeoutId = null;
      }
    });
    ...
    this.recentFilesButton.addEventListener("click", () => {
      this.switchView(SidebarView.FILES);
    });
...
class RecentFilesManager {
    ...
    try {
      ...
      request.onupgradeneeded = event => {
        ...
        if (!db.objectStoreNames.contains(RECENT_FILES_CACHE_STORE)) {
          const store = db.createObjectStore(RECENT_FILES_CACHE_STORE, {
            keyPath: 'fingerprint'
          });
          store.createIndex('cachedAt', 'cachedAt', {
            unique: false
          });
          store.createIndex('fileSize', 'fileSize', {
            unique: false
          });
        }
      };
  ...
  _saveToStorage() {
    try {
      const data = {
        files: Array.from(this.recentFiles.values()).sort((a, b) => b.lastOpened - a.lastOpened)
      };
  ...
  _cleanupOldEntries() {
    ...
    const sortedFiles = Array.from(this.recentFiles.values()).sort((a, b) => b.lastOpened - a.lastOpened);
  ...
  getRecentFiles() {
    return Array.from(this.recentFiles.values()).sort((a, b) => b.lastOpened - a.lastOpened);
  }
...
class SecondaryToolbar {
  ...
  constructor(options, eventBus) {
    ...
    const buttons = [{
      ...
      element: options.toggleAnnotationToolbarButton,
      eventName: "toggleannotationtoolbar",
      close: true
    }, {
...
class SignatureManager {
  ...
  #addToolbarButton(signatureData, uuid, description) {
    ...
    deleteButton.title = "Remove saved signature";
...
class Toolbar {
  #opts;
  #wasLocalized = false;
  #boundListeners = Object.create(null);
  #colorPicker = null;
  constructor(options, eventBus, toolbarDensity = 0) {
    this.#opts = options;
    this.eventBus = eventBus;
    options.editorCursorButton = document.getElementById("editorCursorButton");
    options.editorFreeTextButton = document.getElementById("editorFreeTextButton");
    options.editorHighlightButton = document.getElementById("editorHighlightButton");
    options.editorInkButton = document.getElementById("editorInkButton");
    options.editorStampButton = document.getElementById("editorStampButton");
    options.editorSignatureButton = document.getElementById("editorSignatureButton");
    options.editorFreeTextParamsToolbar = document.getElementById("editorFreeTextParamsToolbar");
    options.editorHighlightParamsToolbar = document.getElementById("editorHighlightParamsToolbar");
    options.editorInkParamsToolbar = document.getElementById("editorInkParamsToolbar");
    options.editorStampParamsToolbar = document.getElementById("editorStampParamsToolbar");
    options.editorSignatureParamsToolbar = document.getElementById("editorSignatureParamsToolbar");
    const buttons = [{
      ...
      element: options.editorCursorButton,
  ...
  setPageScale(pageScaleValue, pageScale) {
    const customScale = Math.round(pageScale * 100);
    ...
    if (!pageScaleValue) {
      const predefinedValueFound = Array.from(scaleSelect.options).some(option => {
        return option.value !== 'custom' && Math.abs(option.value - pageScale) < 0.001;
      });
      if (!predefinedValueFound) {
        customScaleOption.textContent = customScale + '%';
        customScaleOption.value = pageScale;
        customScaleOption.selected = true;
        customScaleOption.hidden = false;
      } else {
        customScaleOption.hidden = true;
      }
    } else {
      customScaleOption.hidden = true;
    }
  ...
  #bindListeners(buttons) {
    ...
    editorStampButton.addEventListener("click", evt => {
  ...
  #editorModeChanged({
    ...
    if (editorCursorButton) {
      editorCursorButton.classList.toggle("toggled", mode === AnnotationEditorType.NONE);
    }
    ...
    editorStampButton.classList.toggle("toggled", mode === AnnotationEditorType.STAMP);
...
const PDFViewerApplication = {
  ...
  async _initializeViewerComponents() {
    ...
    this.recentFilesManager = new RecentFilesManager(eventBus);
    if (appConfig.sidebar?.recentFilesView) {
      this.pdfRecentFilesViewer = new PDFRecentFilesViewer({
        container: appConfig.sidebar.recentFilesView,
        eventBus,
        l10n,
        recentFilesManager: this.recentFilesManager
      });
    }
  ...
  touchPinchCallback(origin, prevDistance, distance) {
    if (this.supportsPinchToZoom) {
      ...
      const PIXELS_PER_LINE_SCALE = 15;
  ...
  async close() {
    ...
    this._hideViewBookmark();
    this._updateRecentFileViewState();
    if (this.pdfRecentFilesViewer) {
      this.pdfRecentFilesViewer.setCurrentFile(null);
    }
    ...
    this._originalFingerprint = null;
  ...
  async open(args) {
    ...
    this._isFromCache = args.isFromCache || false;
    this._originalFingerprint = args.originalFingerprint || null;
    const workerParams = AppOptions.getAll(OptionKind.WORKER);
    Object.assign(GlobalWorkerOptions, workerParams);
    if (args.url && !args.isFromCache) {
  ...
  async download() {
    ...
    if (this._isFromCache) {
      this.downloadManager.downloadData(data, this._docFilename, "application/pdf");
    } else {
  ...
  async save() {
    ...
    try {
      ...
      if (this._isFromCache && this._originalFingerprint && this.recentFilesManager) {
        console.log("Updating cached file data");
        await this.recentFilesManager.updateCachedFileData(this._originalFingerprint, data);
        this.recentFilesManager.updateEditStatus(this._originalFingerprint, false);
  ...
  load(pdfDocument) {
    ...
    this._trackRecentFile(pdfDocument).catch(error => {
      console.warn("Failed to track recent file:", error);
    });
  ...
  _cleanup() {
    this.eventBus.dispatch("cleanup", {
      source: this
    });
  },
  forceRendering() {
  ...
  bindEvents() {
    ...
    if (toggleNotificationsButton) {
      toggleNotificationsButton.addEventListener("click", () => {
        const isEnabled = toggleNotificationsButton.classList.toggle("toggled");
        this.editorUndoBar?.constructor.enableNotifications(isEnabled);
      }, opts);
    }
    this._setupDragAndDrop(opts);
    ...
    eventBus._on("toggleannotationtoolbar", () => {
    ...
    eventBus._on("openrecentfile", this._openRecentFile.bind(this), opts);
    if (this.recentFilesManager) {
      let viewStateUpdateTimeout;
      const throttledViewStateUpdate = () => {
      ...
      eventBus._on("updateviewarea", throttledViewStateUpdate, opts);
      eventBus._on("pagechanging", throttledViewStateUpdate, opts);
      eventBus._on("scalechanging", throttledViewStateUpdate, opts);
      eventBus._on("rotationchanging", throttledViewStateUpdate, opts);
      eventBus._on("annotationeditorstateschanged", this._onAnnotationEditStateChange.bind(this), opts);
    }
  },
  bindWindowEvents() {
  ...
  async _trackRecentFile(pdfDocument) {
    if (!this.recentFilesManager || !pdfDocument) {
      console.log("Recent files manager not available or no document");
      return;
    }
    try {
      ...
      if (!filename) {
        filename = pdfjs_getPdfFilenameFromUrl(this.url);
      }
      ...
      console.log("Tracking recent file:", {
        fingerprint,
        filename,
        url,
        viewState
  ...
  _updateRecentFileViewState() {
    if (!this.recentFilesManager || !this.pdfDocument) {
      return;
    }
    try {
      const fingerprint = this.pdfDocument.fingerprints[0];
      const viewState = this._getCurrentViewState();
      if (viewState.page && viewState.zoom) {
        console.log("Updating view state:", {
          fingerprint,
          viewState
        });
        this.recentFilesManager.updateViewState(fingerprint, viewState);
      }
    } catch (error) {
      console.error("Error updating recent file view state:", error);
    }
  },
  _getCurrentViewState() {
    if (!this.pdfViewer) {
      return {};
    }
    return {
      ...
      scrollLeft: this.pdfViewer.container.scrollLeft,
      rotation: this.pdfViewer.pagesRotation
    };
  },
  async _generateThumbnailForRecentFile(pdfDocument, fingerprint) {
    try {
      const page = await pdfDocument.getPage(1);
      const thumbnailData = await this.recentFilesManager.generateThumbnail(page);
      this.recentFilesManager.updateThumbnail(fingerprint, thumbnailData);
    } catch (error) {
      console.warn("Failed to generate thumbnail for recent file:", error);
    }
  },
  async _openRecentFile(evt) {
    const {
      file
    } = evt;
    if (!file) {
    ...
    try {
      let openArgs = null;
      const cachedFile = await this.recentFilesManager.getCachedFile(file.fingerprint);
      if (cachedFile) {
        console.log("Opening file from cache:", file.filename);
        openArgs = {
        ...
        console.log("Opening file from URL (not cached):", file.filename);
        if (file.url.startsWith('blob:') || file.url.startsWith('file:')) {
          openArgs = {
            url: file.url
          };
        } else {
      ...
      await this.open(openArgs);
      this.eventBus._on("documentloaded", () => {
        setTimeout(() => {
          this._restoreViewState(file.viewState);
        }, 100);
      }, {
        once: true
      });
      ...
      this._otherError("pdfjs-loading-error", {
        message: `Could not open recent file: ${file.filename}`
      });
    }
  },
  _onAnnotationEditStateChange(evt) {
    if (!this.recentFilesManager || !this.pdfDocument) {
      return;
    }
    const fingerprint = this.pdfDocument.fingerprints[0];
    const hasEdits = this.pdfDocument.annotationStorage.size > 0;
    this.recentFilesManager.updateEditStatus(fingerprint, hasEdits);
  },
  _restoreViewState(viewState) {
    if (!viewState || !this.pdfViewer) {
      return;
    }
    if (viewState.page && viewState.page !== this.pdfViewer.currentPageNumber) {
      this.pdfViewer.currentPageNumber = viewState.page;
    }
    if (viewState.zoom && viewState.zoom !== this.pdfViewer.currentScale) {
      this.pdfViewer.currentScale = viewState.zoom;
    }
    if (viewState.scrollTop !== undefined || viewState.scrollLeft !== undefined) {
      this.pdfViewer.container.scrollTop = viewState.scrollTop || 0;
      this.pdfViewer.container.scrollLeft = viewState.scrollLeft || 0;
    }
    if (viewState.rotation !== undefined && viewState.rotation !== this.pdfViewer.pagesRotation) {
      this.pdfViewer.pagesRotation = viewState.rotation;
    }
  },
  _setupDragAndDrop(opts) {
    const dropZone = this.appConfig.appContainer;
...
function onWheel(evt) {
  ...
  if (isPinchToZoom || evt.ctrlKey && supportsMouseWheelZoomCtrlKey || evt.metaKey && supportsMouseWheelZoomMetaKey) {
    evt.preventDefault();
    if (document.visibilityState === "hidden" || this.overlayManager.active) {
      return;
    }
...
function onClick(evt) {
  ...
  if (this.pdfViewer && this.pdfViewer.annotationEditorMode !== AnnotationEditorType.NONE) {
    const clickedElement = evt.target;
    console.log('PDF.js Background Click Debug:', {
      currentMode: this.pdfViewer.annotationEditorMode,
      clickTarget: clickedElement,
      clickTargetClass: clickedElement.className,
      clickTargetId: clickedElement.id,
      clickTargetTagName: clickedElement.tagName
    });
    if (isBackgroundClick(clickedElement, this.appConfig)) {
      console.log('PDF.js: Background click detected, switching to normal mode');
      this.eventBus.dispatch("switchannotationeditormode", {
        source: this,
        mode: AnnotationEditorType.NONE
      });
    } else {
      console.log('PDF.js: Not a background click, keeping current mode');
    }
  }
}
function isBackgroundClick(target, appConfig) {
  console.log('PDF.js isBackgroundClick check:', target, 'ID:', target.id, 'Class:', target.className);
  if (target.matches('input, textarea, button, select')) {
    console.log('PDF.js: Rejected - form element');
    return false;
  }
  if (target.closest('#floatingAnnotationToolbar, .toolbar, #secondaryToolbar')) {
    console.log('PDF.js: Rejected - toolbar element');
    return false;
  }
  if (target.closest('.page')) {
    console.log('PDF.js: Rejected - page content');
    return false;
  }
  if (target.closest('.annotationEditorLayer')) {
    console.log('PDF.js: Rejected - annotation editor layer');
    return false;
  }
  if (target.id === 'viewerContainer' || target.id === 'viewer') {
    console.log('PDF.js: Accepted - clicked on viewer background!');
    return true;
  }
  if (target.classList && target.classList.contains('pdfViewer')) {
    console.log('PDF.js: Accepted - clicked on pdfViewer background!');
    return true;
  }
  console.log('PDF.js: Rejected - not a background click');
  return false;
}
...
function initializeMainToolbarPageLayoutButtons() {
  ...
  const buttonMappings = [{
    button: scrollVerticalMain,
    mode: ScrollMode.VERTICAL
  }, {
    button: scrollHorizontalMain,
    mode: ScrollMode.HORIZONTAL
  }, {
    button: scrollWrappedMain,
    mode: ScrollMode.WRAPPED
  }, {
    button: scrollPageMain,
    mode: ScrollMode.PAGE
  }];
  ...
  function syncButtonStates() {
    ...
    if (app?.eventBus) {
      app.eventBus._on('scrollmodechanged', ({
        mode
      }) => {
