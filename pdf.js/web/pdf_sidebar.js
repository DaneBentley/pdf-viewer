 ...
 *   the layers view.
 * @property {HTMLButtonElement} recentFilesButton - The button used to show
 *   the recent files view.
 ...
 * @property {HTMLDivElement} recentFilesView - The container in which
 *   the recent files are placed.
...
class PDFSidebar {
  ...
  // Minimum time in milliseconds that the mouse must hover before opening
  #HOVER_DELAY = 200;
  ...
  reset() {
    ...
    // Hide all optional view buttons initially until content is loaded
  ...
  switchView(view, forceOpen = false) {
    ...
    switch (view) {
      ...
      case SidebarView.FILES:
        // Recent files view is always available
        break;
      default:
  ...
  open() {
    ...
    // Ensure the sidebar is visible when opening
  ...
  #addEventListeners() {
    ...
    // Track mouse position relative to sidebar
    ...
    this.sidebarContainer.addEventListener("mouseleave", (evt) => {
      // Check if we're moving to the resizer
      ...
      if (this.#isTemporarilyOpen) {
        this.#hoverTimeoutId = setTimeout(() => {
          // Only close if mouse is not in sidebar or resizer
    ...
    // Track mouse position relative to resizer with hover delay
    this.resizer.addEventListener("mouseenter", () => {
      ...
      if (!this.isOpen && !this.#isTemporarilyOpen) {
        // Add a longer delay before opening to prevent accidental triggers
        this.#hoverTimeoutId = setTimeout(() => {
    ...
    this.resizer.addEventListener("mouseleave", (evt) => {
      ...
      // Add a small delay before closing to make interaction smoother
      if (this.#isTemporarilyOpen) {
        this.#hoverTimeoutId = setTimeout(() => {
          // Only close if mouse is not in sidebar or resizer
          if (!this.#isMouseInSidebar && !this.#isMouseInResizer) {
    ...
    this.sidebarContainer.addEventListener("click", () => {
      // No longer converting temporary to permanent on click
      // Just reset any pending close timeout to allow interaction
    ...
    // Handle scroll events in the sidebar
    ...
    this.toggleButton.addEventListener("click", evt => {
      // Clear any hover timeouts when manually toggling
      if (this.#hoverTimeoutId) {
        clearTimeout(this.#hoverTimeoutId);
        this.#hoverTimeoutId = null;
      }
      // Convert temporary to permanent only when explicitly toggled
      if (this.#isTemporarilyOpen) {
        this.#isTemporarilyOpen = false;
      }
      this.toggle(evt);
    ...
    // Hide/show views based on availability.
    const onTreeLoaded = (count, button, view) => {
      // Hide the button if no content is available, show it if content exists
      if (count) {
