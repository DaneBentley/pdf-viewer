...
const PDFViewerApplication = {
  ...
  /** @type {PDFLayerViewer} */
  pdfLayerViewer: null,
  /** @type {PDFRecentFilesViewer} */
  pdfRecentFilesViewer: null,
  /** @type {RecentFilesManager} */
  ...
  async _initializeViewerComponents() {
    ...
    // Initialize recent files manager and viewer
  ...
  async close() {
    ...
    // Update recent file view state before closing
  ...
  async open(args) {
    ...
    // Track if this file is opened from cache
    ...
    if (typeof PDFJSDev !== "undefined" && PDFJSDev.test("MOZCENTRAL")) {
    ...
    } else if (args.url && !args.isFromCache) {
      // Only set download URL for non-cached files
      ...
      // For cached files, just set the title without a download URL
  ...
  async download() {
    ...
    if (this._isFromCache) {
      // For cached files, always download the current data
      this.downloadManager.downloadData(data, this._docFilename, "application/pdf");
    } else {
      // For non-cached files, use the original download logic
  ...
  async save() {
    ...
    try {
      ...
      if (this._isFromCache && this._originalFingerprint && this.recentFilesManager) {
        // For cached files, update the cache instead of downloading
        console.log("Updating cached file data");
        await this.recentFilesManager.updateCachedFileData(this._originalFingerprint, data);
        
        // Also update the recent files entry to mark it as having edits
        this.recentFilesManager.updateEditStatus(this._originalFingerprint, false);
        
        // Show a success message instead of downloading
        console.log("File saved to cache successfully");
      } else if (this._isFromCache) {
        // Cached file but no recent files manager - just download
        this.downloadManager.downloadData(data, this._docFilename, "application/pdf");
      } else {
        // Non-cached files use original download logic
        this.downloadManager.download(data, this._downloadUrl, this._docFilename);
      }
    } catch (reason) {
  ...
  load(pdfDocument) {
    ...
    // Track this file in recent files (async, don't wait)
   ...
   */
  _cleanup() {
    this.eventBus.dispatch("cleanup", { source: this });
  },
  ...
  bindEvents() {
    ...
    // Add notifications toggle handler
    ...
    if (typeof PDFJSDev !== "undefined" && PDFJSDev.test("MOZCENTRAL")) {
      ...
      eventBus._on(
        "reporttelemetry",
        evt => externalServices.reportTelemetry(evt),
        opts
      );
    }
    if (
    ...
    // Recent files event handlers
    eventBus._on("openrecentfile", this._openRecentFile.bind(this), opts);
    
    // Track view state changes for recent files (only if recentFilesManager exists)
  ...
  /**
   * Track a PDF file in recent files with caching
   * @private
   */
  ...
  /**
   * Update view state for current file in recent files
   * @private
   */
  _updateRecentFileViewState() {
    ...
    try {
      ...
      // Only update if we have meaningful view state data
      if (viewState.page && viewState.zoom) {
        console.log("Updating view state:", { fingerprint, viewState });
  ...
  /**
   * Generate thumbnail for recent file
   * @private
   */
  async _generateThumbnailForRecentFile(pdfDocument, fingerprint) {
    try {
      // Wait for first page to be available
      const page = await pdfDocument.getPage(1);
      
      // Generate thumbnail using the recent files manager
      const thumbnailData = await this.recentFilesManager.generateThumbnail(page);
      
      // Update the thumbnail in recent files
  ...
  /**
   * Open a recent file using cached data when available
   * @private
   */
  async _openRecentFile(evt) {
    ...
    try {
      ...
      if (cachedFile) {
        ...
        // Open directly with data to avoid blob URL issues
        openArgs = {
      ...
      // Restore view state after document loads
      this.eventBus._on("documentloaded", () => {
        setTimeout(() => {
          this._restoreViewState(file.viewState);
        }, 100); // Small delay to ensure document is fully loaded
      }, { once: true });
    } catch (error) {
      console.error("Failed to open recent file:", error);
      // If opening fails, show an error message
      this._otherError("pdfjs-loading-error", { message: `Could not open recent file: ${file.filename}` });
    }
  },
  ...
  /**
   * Handle annotation edit state changes
   * @private
   */
  ...
  /**
   * Restore view state from recent file
   * @private
   */
  ...
  /**
   * Setup drag and drop support for PDF files
   * @private
   */
    ...
    dropZone.addEventListener('drop', (e) => {
      ...
      if (pdfFiles.length > 0) {
        ...
        this.open({
          ...
          // File will be automatically tracked in recent files via _trackRecentFile
        }).catch(error => {
...
function onClick(evt) {
  ...
  // Check if this is a background click that should reset to normal mode
...
/**
 * Determines if a click event occurred on the viewer background area
 * and should trigger a switch to normal mode.
 */
...
function onKeyDown(evt) {
  ...
  if (cmd === 1 || cmd === 8 || cmd === 5 || cmd === 12) {
    ...
    switch (evt.keyCode) {
      case 70: // f
        // Always prevent find functionality
        evt.preventDefault();
        handled = true;
        break;
      case 71: // g
